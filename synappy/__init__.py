...
class synwrapper(object):
    ...
    def add_stim_on(self, event_type = 'stim', stim_thresh = 2, spont_filtsize = 1001,
        spont_threshampli = 3, spont_threshderiv = -1.2, spont_thresh_pos_deriv = 0.1, savgol_polynomial = 3):
        ...
        if event_type == 'stim':
            ...
            stim_on = find_spontaneous(analogs, filt_size = spont_filtsize,
                thresh_ampli = spont_threshampli, thresh_deriv = spont_threshderiv,
...
def find_spontaneous(analog_signals, filt_size = 501, thresh_ampli = 3, thresh_deriv = -1.2, thresh_pos_deriv = 0.1, savgol_polynomial = 3):
    num_neurons = len(analog_signals)
    stim_on = np.empty(num_neurons, dtype = np.ndarray)
    ...
    for neuron in range(num_neurons):
        ...
        for trial in range(num_trials):
            ...
            event_start = deriv_lessthan[deriv_lessthan - deriv_lessthan_shifted > 1]
            event_start = np.ma.array(event_start.copy(), mask = np.zeros(len(event_start)))
            ...
            event_end = deriv_greaterthan[deriv_greaterthan - deriv_greaterthan_shifted > 1]

            #Set the initial current_event_finish: the first value in event_end falling
            #after the first event_start.
            current_event_finish_ind = np.where(event_end > event_start[0])[0][0]
...
def export(files, channels = 'all', filetype = 'matlab', names = None):
    ...
    for neuron in range(num_neurons):
        ...
        if filetype == 'matlab':
            sp.io.savemat(current_name, {'analogSignals':analog_signals[neuron],'times':times[neuron]}, do_compression=True)
